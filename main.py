...
def get_data_tuple(splits: str, bs:int, shuffle=False, drop_last=False) -> DataTuple:
    ...
    tset = VQATorchDataset(dset, args.model)
...
if __name__ == "__main__":
    ...
    if args.test is not None:
        ...
        if 'test' in args.test:
            ...
            data = json.load(a)
            b = open("models/trained/minival_predict.json")
            predictions = json.load(b)

            images_ids=[]
            for i in range(len(data)):
                if data[i]["img_id"] not in images_ids:
                    images_ids.append(data[i]["img_id"])
